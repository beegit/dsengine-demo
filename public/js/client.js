...
/***/ function(module, exports, __webpack_require__) {
	...
	var DseClient = __webpack_require__(1).Client;
	var _ = __webpack_require__(4);
	var USE_DELTA_PATCHING = __webpack_require__(15).USE_DELTA_PATCHING;
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(module) {var _ = __webpack_require__(4);
	var ClientState = __webpack_require__(5);
	var EditPacket = __webpack_require__(8);
	var DIFF_EQUAL = 0;
	//var DIFF_DELETE = -1;
	//var DIFF_INSERT = 1;
	 ...
	 * @param {Function} [options.onRawEditApplied] Event fires when a raw edit is applied.
	 */
	var DsEngineClient = function DsEngineClient(options) {
	  ...
	  this.state = new ClientState({
	    ...
	    onRawEditApplied: function() {
	      if (_.isFunction(options.onRawEditApplied)) {
	        options.onRawEditApplied(arguments);
	      }
	    }
	  });
	};
	...
	DsEngineClient.prototype.startSync = function startSync(options) {
	  ...
	  var editPacket = new EditPacket({
	    v: this.state.shadow.serverVersion,
	    editStack: this.state.editStack
	  });

	  if (this.state.shadow.deltaOk) {
	    // The last delta postback from the server was successful, so
	    // sync in delta mode.
	    var diffs = this.state.calcDiff();
	    var changed = diffs.length !== 1 || diffs[0][0] !== DIFF_EQUAL;

	    if (changed) {
	      this.state.copyDocToShadow();
	    }

	    if (changed || !this.state.editStack.length) {
	      // Drop the edit onto the edit stack
	      editPacket.addEdit({
	        mode: "d",
	        clientVersion: this.state.shadow.clientVersion,
	        serverVersion: this.state.shadow.serverVersion,
	        patch: (this.useDeltaPatching) ?
	          this.state.diffsToDelta(diffs) :
	          this.state.diffsToTextPatches(diffs)
	      });

	      this.state.shadow.clientVersion++;
	    }
	  } else {
	    // The last delta postback from the server either hasn't happened
	    // or was unsuccessful, so sync in raw mode to get in sync.
	    this.state.shadow.text = this.state.getDocText();
	    this.state.shadow.clientVersion++;
	    ...
	    this.state.shadow.deltaOk = true;
	  }
	...
	/**
	 * Handles a server response which is the back half of
	...
	DsEngineClient.prototype.receiveEdits = function receiveEdits(editPacket) {
	  ...
	  // been processed
	  this.state.editStack = _.reject(this.state.editStack, function(edit) {
	    return edit.clientVersion <= editPacket.v;
	  });

	  // Apply any new edits coming from the server
	  if (editPacket.editStack && editPacket.editStack.length > 0) {
	    this.state.applyEdits(editPacket);
	  }
	...
	/**
	 * Handle a failed sync loop if we're told that it failed. This
	...
	/**
	 * Helper for getting shadow text.
	 *
	 * @returns {ClientState.shadow.text|*}
	 */
	DsEngineClient.prototype.getShadowText = function() {
	  return this.state.shadow.text;
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ = __webpack_require__(4);
	var Dmp = __webpack_require__(6);
	var dmp = new Dmp();

	/**
	 * ClientState encapsulates the text and shadows for each client instance
	 ...
	 * @param {Boolean} [options.useDeltaPatching=true] TRUE for delta, FALSE for text
	 * @param {Function} [options.onPatchFailed] Event fires when a patch fails
	 * @param {Function} [options.onRawEditApplied] Event fires when a raw edit is applied
	 * @param {Function} [options.setDocText] Function to override setting doc text
	 * @param {Function} [options.getDocText] Function to override getting doc text
	 * @returns {ClientState}
	 */
	var ClientState = function ClientState(options) {
	  options = (options || {});
	  var defaults = {
	    useDeltaPatching: true,
	    doc: "",
	  ...
	  this.shadow = {
	    deltaOk: options.shadow.deltaOk,
	  ...
	  this.onPatchFailed = options.onPatchFailed;
	  this.onRawEditApplied = options.onRawEditApplied;

	  return this;
	};

	ClientState.prototype.getDocText = function getDocText() {
	  return this.doc;
	};

	ClientState.prototype.setDocText = function getDocText(docText) {
	  this.doc = docText;
	};
	 ...
	 *
	 * @param {Array} editPacket The editPacket to patch on top of the shadow.
	 */
	ClientState.prototype.applyEdits = function applyEdits(editPacket) {
	  var _this = this;

	  _.each(editPacket.editStack, function(edit) {
	    switch (edit.mode) {
	      case "r":
	...
	ClientState.prototype.applyRawEdit = function applyRawEdit(edit) {
	  // Apply a merge raw edit. Reset the versions and set the shadow
	  // to match the server. Whatever is in client text will be merged
	  // on the following sync loop.
	  this.shadow.text = edit.doc;
	  this.shadow.clientVersion = edit.clientVersion;
	  this.shadow.serverVersion = edit.serverVersion;
	  this.editStack = [];
	  if (_.isFunction(this.onRawEditApplied)) {
	    this.onRawEditApplied();
	  }
	};

	ClientState.prototype.applyDeltaEdit = function applyDeltaEdit(edit) {
	  var _this = this;
	  var diffs;
	  var patches;
	  ...
	  if (edit.clientVersion !== _this.shadow.clientVersion) {
	    ...
	    _this.shadow.deltaOk = false;
	    console.warn("Client version number mismatch.\n" +
	    ...
	    _this.shadow.deltaOk = false;
	    console.warn("Server version in future.\n" +
	      "Expected: " + _this.shadow.serverVersion + " " +
	      "Got: " + edit.serverVersion);
	  } else if (edit.serverVersion < _this.shadow.serverVersion) {
	    ...
	    if (_this.useDeltaPatching) {
	      try {
	        ...
	        console.warn("Delta patch failed: " + e.message);
	        _this.shadow.deltaOk = false;
	        if (_.isFunction(this.onPatchFailed)) {
	          this.onPatchFailed();
	        }
	      }
	    } else {
	 ...
	 */
	ClientState.prototype.patchDoc = function patchDoc(patches) {
	  var tmpDoc = this.getDocText();
	  var result = dmp.patch_apply(patches, tmpDoc);
	  // Set the new text only if there is a change to be made.
	  if (tmpDoc != result[0]) {
	    // Override setDocText to catch this text and set it
	    // in the client UI in an appropriate fashion.
	 ...
	 */
	ClientState.prototype.calcDiff = function calcDiff() {
	 ...
	 */
	ClientState.prototype.applyPatches = function applyPatches(textPatches) {
	 ...
	 */
	ClientState.prototype.diffsToPatches = function diffsToPatches(diffs) {
	 ...
	 */
	ClientState.prototype.diffsToTextPatches = function diffsToTextPatches(diffs) {
	  if (!diffs || diffs.length < 1) {
	    return "";
	  }
	 ...
	 */
	ClientState.prototype.diffsToDelta = function diffsToDelta(diffs) {
	  if (!diffs || diffs.length < 1) {
	    return "";
	  }
	 ...
	 */
	ClientState.prototype.copyDocToShadow = function copyDocToShadow() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__(7).diff_match_patch;
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * @param {Number} data.v When `client=>server`, the serverVersion. When
	 *                        `server=>client`, the clientVersion.
	 * @param {Array}  data.editStack Stack of edits to apply
	 * @returns {EditPacket}
	 */
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ = __webpack_require__(4);
	var EditPacket = __webpack_require__(8);
	var ServerState = __webpack_require__(13);
	...
	var DsEngineServer = function DsEngineServer(options) {
	  ...
	  if (options.client) {
	    dbOptions.client = options.client;
	  }

	  this.clientId = options.clientId;
	  this.docId = options.docId;
	  this.db = options.db(dbOptions);
	  this.docInit = options.docInit;
	  this.useDeltaPatching = options.useDeltaPatching;
	};

	/**
	 *  Receive edits from the client and process them against the database.
	 *
	 *  @param {EditPacket} [editPacket] Optional edit packet to process.
	 */
	DsEngineServer.prototype.receiveEdits = function receiveEdits(editPacket, cb) {
	  if (_.isFunction(editPacket)) {
	    cb = editPacket;
	    editPacket = null;
	  }
	  ...
	  async.waterfall([
	    ...
	    function(done) {
	      _this.db.get(function(err, state) {
	        if (err) {
	          return done(err);
	        }
	        ...
	        async.waterfall([
	            ...
	            // db. If it is, use it.
	            function(done2) {
	              if (_.isUndefined(state.doc) ||  _.isNull(state.doc)) {
	                return done2(null);
	              }
	          ...
	          function(err) {
	            if (err && err !== "ok") {
	              console.error("[dsengine] Receive Edits", err);
	              return done(err);
	            }

	            done(null, new ServerState(
	              _.extend(state, {
	    ...
	    function(state, done) {
	      if (editPacket.v !== state.shadow.serverVersion &&
	          editPacket.v === state.shadow.backupServerVersion) {
	        console.warn("[dsengine] Rollback from shadow " +
	          state.shadow.serverVersion + " to backup shadow " +
	          state.shadow.backupServerVersion);
	        state.shadow.text = state.shadow.backupText;
	        state.shadow.serverVersion = state.shadow.backupServerVersion;
	    ...
	    function(state, done) {
	      _this.db.getEditStack(function(err, editStack) {
	        done(err, state, editStack);
	      });
	    },

	    // Remove anything from the edit stack that has been
	    // ack'd by the client.
	    function(state, editStack, done) {
	    ...
	    function(state, done) {
	      state.applyEdits(editPacket);
	      _this.db.set(state, done);
	    }
	  ...
	  async.waterfall([
	    ...
	    function(done) {
	      _this.db.get(function(err, state) {
	        ...
	        done(null, new ServerState(_.extend(state, {
	          useDeltaPatching: _this.useDeltaPatching
	        })));
	      });
	    },
	    ...
	    function(state, editStack, done) {
	      var editPacket = new EditPacket({
	        v: state.shadow.clientVersion,
	        editStack: editStack
	      });

	      if (state.shadow.deltaOk) {
	        var diffs = state.calcDiff();

	        editPacket.addEdit({
	          mode: "d",
	          clientVersion: state.shadow.clientVersion,
	          serverVersion: state.shadow.serverVersion,
	          patch: (_this.useDeltaPatching) ?
	            state.diffsToDelta(diffs) :
	            state.diffsToTextPatches(diffs)
	        });

	        state.shadow.serverVersion++;
	      } else {
	        state.shadow.deltaOk = true;
	        state.shadow.clientVersion++;
	        editPacket.addEdit({
	          mode: "r",
	          clientVersion: state.shadow.clientVersion,
	          serverVersion: state.shadow.serverVersion,
	      ...
	      state.copyDocToShadow();

	      done(null, editPacket, state);
	    },

	    function(editPacket, state, done) {
	      _this.db.set(state, function(err) {
	        if (err) {
	          return done(err);
	        }
	    ...
	    function(err, editPacket, state) {
	      if (err && err !== "ok") {
	        return cb(err, false);
	      }

	      cb(null, editPacket, state.doc);
	    }

	  );
	};

	module.exports = DsEngineServer;
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(11), __webpack_require__(12).setImmediate))
...
/***/ function(module, exports, __webpack_require__) {
	...
	/**
	 * ServerState encapsulates the text and shadows for each client instance
	 * and handles diffing/patching responsibilites.
	 *
	 * @param {Object} options The client data options hash
	 ...
	 * @returns {ServerState}
	 */
	var ServerState = function ServerState(options) {
	  options = (options || {});
	  ...
	  if (options.shadow) {
	    ...
	    options.shadow.deltaOk =
	      (options.shadow.deltaOk !== false && options.shadow.deltaOk !== "false");
	  }
	 ...
	 * @returns {ServerState}
	 */
	ServerState.prototype.applyEdits = function applyEdits(editPacket) {
	  var _this = this;
	 ...
	 */
	ServerState.prototype.applyRawEdit = function applyRawEdit(edit) {
	  this.shadow.deltaOk = true;
	  this.shadow.text = edit.doc;
	  this.shadow.clientVersion = edit.clientVersion;
	  this.shadow.serverVersion = edit.serverVersion;
	 ...
	 * @param {Object} edit The edit to apply.
	 */
	ServerState.prototype.applyDeltaEdit = function applyDeltaEdit(edit) {
	  var diffs;
	  var patches;

	  if (edit.serverVersion !== this.shadow.serverVersion) {
	    // Can't apply a delta on a mismatched shadow version, send a raw sync
	    this.shadow.deltaOk = false;
	    console.warn("[dsengine] Shadow version mismatch: " + edit.serverVersion +
	      " != " + this.shadow.serverVersion);
	  } else if (edit.clientVersion > this.shadow.clientVersion) {
	    // Client has a newer version than the server, send a raw sync
	    this.shadow.deltaOk = false;
	    console.warn("[dsengine] Future delta: " + edit.clientVersion +
	      " > " + this.shadow.clientVersion);
	  } else if (edit.clientVersion < this.shadow.clientVersion) {
	    // We've already seen this diff. Pass.
	    console.warn("[dsengine] Repeated delta: " + edit.clientVersion +
	      " < " + this.shadow.clientVersion);
	  } else {
	    if (this.useDeltaPatching) {
	      try {
	        ...
	        this.shadow.deltaOk = false;
	        console.warn("[dsengine] Delta patch failed", e);
	      }
	    } else {
	      patches = dmp.patch_fromText(edit.patch);
	    }
	 ...
	 * @param {Patch[]} patches The patches to apply.
	 */
	ServerState.prototype.patchDoc = function patchDoc(patches) {
	  var tmpDoc = this.doc;
	  var result = dmp.patch_apply(patches, tmpDoc);
	  // Set the new text only if there is a change to be made.
	  if (tmpDoc != result[0]) {
	 ...
	 * @returns {Diff[]} Array of diffs.
	 */
	ServerState.prototype.calcDiff = function calcDiff() {
	 ...
	 * @param {Patch[]} textPatches The text patches to apply.
	 */
	ServerState.prototype.applyPatches = function applyPatches(textPatches) {
	  this.shadow.text = dmp.patch_apply(textPatches, this.shadow.text)[0];
	  return this;
	};
	 ...
	 * @returns {Patch[]} Array of patches.
	 */
	ServerState.prototype.diffsToPatches = function diffsToPatches(diffs) {
	  if (!diffs || diffs.length < 1) {
	    return [];
	  }
	  return dmp.patch_make(diffs);
	 ...
	 * @returns {String} Text patch.
	 */
	ServerState.prototype.diffsToTextPatches = function diffsToTextPatches(diffs) {
	  if (!diffs || diffs.length < 1) {
	    return "";
	  }
	  var patches = this.diffsToPatches(diffs);
	 ...
	 * @returns {String} Encoded string representing the delta.
	 */
	ServerState.prototype.diffsToDelta = function diffsToDelta(diffs) {
	  if (!diffs || diffs.length < 1) {
	    return "";
	  }
	  return dmp.diff_toDelta(diffs);
	 ...
	 * Copy the document text into the shadow text.
	 */
	ServerState.prototype.copyDocToShadow = function copyDocToShadow() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  isNoopEdit: function(editPacket) {
	    var isNoop = true;

	    if (_.some(editPacket.editStack, function(edit) {
	      return edit.mode !== "r" && !/^=\d+$/.test(edit.patch);
	    })) {
	      isNoop = false;
	    }
